...
pub fn parse_c_string(ptr: *const c_char, field_name: &str) -> Result<String, *mut ApiResult> {
    ...
    unsafe {
        match CStr::from_ptr(ptr).to_str() {
            Ok(s) => Ok(s.to_string()),
            Err(_) => Err(Box::into_raw(Box::new(ApiResult::error(format!(
                "Invalid UTF-8 string for {}",
                field_name
            ))))),
        }
    }
...
pub fn validate_config_json(config_str: &str) -> Result<(), *mut ApiResult> {
    if config_str.is_empty() {
        return Err(Box::into_raw(Box::new(ApiResult::error(
            "Config string is empty".to_string(),
        ))));
    }
